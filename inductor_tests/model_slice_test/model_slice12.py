...
torch._inductor.config.post_grad_custom_post_pass = torch_npu._inductor.fx_passes.post_custom_passes.run_register_post_custom_passes
...
class Repro(torch.nn.Module):
    ...
    def forward(self, arg0_1, arg1_1, arg2_1, arg3_1):
        full_default = torch.ops.aten.full.default([bs, 38], 0, dtype = torch.int64, layout = torch.strided, device = device(type='npu', index=0), pin_memory = False)
...
def load_args(reader):
    buf0 = reader.storage(None, bs*38*8, device=device(type='npu', index=0), dtype_hint=torch.int64)
    reader.tensor(buf0, (bs, 38), dtype=torch.int64, is_leaf=True)  # arg0_1
    buf1 = reader.storage(None, bs*38, device=device(type='npu', index=0), dtype_hint=torch.bool)
    reader.tensor(buf1, (bs, 38), dtype=torch.bool, is_leaf=True)  # arg1_1
    buf2 = reader.storage(None, 20969728, device=device(type='npu', index=0))
    reader.tensor(buf2, (81913, 64), is_leaf=True)  # arg2_1
    buf3 = reader.storage(None, bs*38*4, device=device(type='npu', index=0))
