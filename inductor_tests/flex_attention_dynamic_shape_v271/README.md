...
`M01`–`M16` 是 patch 随附的社区回归用例。它们从已安装的 `torch_npu` 定位源码，检查 DynamicScalar、offset/mapping、旧 metadata 清理、backward 任务数和动态 forward/backward 复用。
...
python run_flex_attention_tests.py --case M14_forward_reuses_graph_for_dynamic_q_kv_and_sparse_z
```
...
返回码为 `0` 表示没有 FAIL/TIMEOUT；有 FAIL/TIMEOUT 时返回 `1`，便于 CI 或批量服务器脚本判断。无 NPU 时 unittest 的社区用例会显示为 `SKIP`，主验收用例会直接失败，避免把未实际执行的验收误报为全通过。
